...
$container['cache'] = function ($c) {
    ...
    $server = $config->get('cache.memcacheServer');

    $memcache = new \Memcache;
    $cache    = new \SlimRouter\Cache;

    $cache->setAdapter($memcache);

    foreach ($server as $instance) {
        $cache->addServer($instance['host'], $instance['port'], $instance['persistent'], $instance['weight']);
    }
